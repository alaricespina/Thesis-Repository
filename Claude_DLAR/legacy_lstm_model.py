"""
Legacy LSTM Auto Regressive Model

LSTM variant of the custom Random Forest implementation with identical deseasonalization approach.
...
- Windowed autoregressive features (adapted for LSTM)
- Stochastic noise injection during prediction
- Full seasonal reconstruction

The only change is replacing RandomForestRegressor with LSTM while keeping everything else identical.
"""
...
class LegacyLSTMAR:
    """
    Legacy LSTM with custom deseasonalization exactly as originally implemented.
    
    This model performs identical preprocessing to Legacy RF:
    1. Remove yearly seasonality using configurable cycle length
    2. Remove monthly seasonality 
    3. Train LSTM on windowed residuals (instead of Random Forest)
    ...
    def __init__(self, model_order=3, yearly_season_length=7):
        """
        Initialize Legacy LSTM model
        
        Args:
            model_order: Number of lagged values to use for prediction (LSTM timesteps)
    ...
    def remove_yearly_mean(self, df, col):
        ...
        IDENTICAL to Legacy RF implementation
    ...
    def window_data(self, data):
        """
        Create windowed features for LSTM (adapted from Legacy RF)
        
        Args:
            data: 1D array of time series data
            
        Returns:
            Tuple of (X, y) where X is 3D for LSTM [samples, timesteps, features], y is targets
        """
        n = len(data)
        x = []
        y = []
        ...
        # Reshape X to 3D: [samples, timesteps, 1 feature]
    ...
    def split_data(self, df, split_percent=70):
        ...
        IDENTICAL logic to Legacy RF but adapted for LSTM format
        ...
        # Create windowed features (now 3D for LSTM)
    ...
    def build_lstm_model(self, input_shape):
        ...
        Args:
            input_shape: Tuple of (timesteps, features) for LSTM input
    ...
    def fit(self, data, target_columns):
        """
        Fit Legacy LSTM models for each target variable
        IDENTICAL preprocessing to Legacy RF, only model training changed
        ...
        for col in target_columns:
            if col in data.columns:
                print(f"Training Legacy LSTM for {col}...")
                
                # IDENTICAL DATA PREPARATION TO LEGACY RF
                # Prepare input data
                ...
                # Store model and parameters (IDENTICAL TO LEGACY RF)
                ...
                print(f"  ✓ Legacy LSTM trained for {col}")
    ...
    def do_predictions(self, train_x_last, num_predictions, col, year_std_scale=0.1, month_std_scale=0.1):
        ...
        IDENTICAL logic to Legacy RF but adapted for LSTM.predict()
        
        Args:
            train_x_last: Last training window for initialization (3D for LSTM)
        ...
        preds = np.array([])
        r_queue = np.copy(train_x_last)  # Shape: (timesteps, 1)
        
        # Generate predictions with noise injection (IDENTICAL LOGIC TO LEGACY RF)
        for i in range(num_predictions):
            # Reshape for LSTM input: (1, timesteps, features)
            tr = r_queue.reshape(1, r_queue.shape[0], r_queue.shape[1])
            
            # Add stochastic residuals (IDENTICAL TO LEGACY RF)
            ...
            # Predict and add noise (adapted for LSTM)
            next_val = model.predict(tr, verbose=0)[0][0] + month_resid + year_resid
            preds = np.append(preds, next_val)
            
            # Update queue for next prediction (IDENTICAL LOGIC)
            r_queue = np.roll(r_queue, -1, axis=0)
            r_queue[-1, 0] = next_val
    ...
    def reconstruct_predictions(self, preds, col, start_date):
        ...
        # Step 1: Reconstruct monthly component (IDENTICAL)
        ...
        # Step 2: Reconstruct yearly component (IDENTICAL)
    ...
    def predict_n_years(self, target_columns, n_days):
        ...
        IDENTICAL interface and logic to Legacy RF
        ...
        for col in target_columns:
            if col in self.models:
                try:
                    ...
                    # Reconstruct with full seasonality (IDENTICAL)
