#!/usr/bin/env python3
"""
Deep Learning Auto Regressive (DLAR) Models
Comprehensive implementation of AR models for weather prediction

Models implemented:
1. Vanilla Auto Regressive (Linear Regression)
2. ARIMA 
3. SARIMA
4. Deep Learning AR (LSTM with deseasonalization)
...
from deep_learning_ar import DeepLearningAutoRegressive
...
class DLARRunner:
    ...
    def train_deep_learning_ar(self):
        """Train Deep Learning AR model with fallback"""
        print("\n" + "="*50)
        print("Training Deep Learning Auto Regressive Model")
        print("="*50)
        
        try:
            print("Attempting TensorFlow LSTM model...")
            model = DeepLearningAutoRegressive(
                sequence_length=30, 
                lstm_units=32, 
                batch_size_pred=2000  # Larger batch size for faster predictions
            )
            model.fit(self.historical_targets, self.target_columns, epochs=50, batch_size=32)
            
            # Check if at least one model was trained
            if any(col in model.models for col in self.target_columns):
                self.models['Deep_Learning_AR'] = model
                print("✓ TensorFlow LSTM Deep Learning AR trained successfully")
                return
            else:
                print("✗ TensorFlow LSTM failed - trying Simple Deep AR fallback...")
                
        except Exception as e:
            print(f"✗ TensorFlow LSTM failed: {e}")
        ...
        try:
            ...
            if any(col in simple_model.models for col in self.target_columns):
                self.models['Deep_Learning_AR'] = simple_model
        ...
        try:
            ...
            if any(col in fast_model.patterns for col in self.target_columns):
                self.models['Deep_Learning_AR'] = fast_model
                print("✓ Fast Deep AR fallback trained successfully")
            else:
                print("✗ All Deep Learning methods failed - skipping Deep Learning AR")
                
        except Exception as e:
            print(f"✗ All Deep Learning methods failed: {e}")
            print("Skipping Deep Learning AR model entirely")
    ...
    def generate_long_term_predictions(self):
        ...
        for model_name, model in self.models.items():
            ...
            elif model_name == 'Deep_Learning_AR':
                predictions = model.predict_n_years(self.historical_targets, self.target_columns, self.prediction_days)
            else:  # ARIMA, SARIMA
    ...
    def save_predictions(self):
        ...
        for model_name, predictions in self.predictions_long_term.items():
            ...
            filename = f"{pred_dir}/{model_name}_{self.prediction_years}_year_predictions.csv"
        ...
        if hasattr(self, 'test_predictions'):
            ...
            for model_name, predictions in self.test_predictions.items():
                ...
                df = pd.DataFrame(data_dict)
                filename = f"{pred_dir}/{model_name}_2025_test_predictions.csv"
                df.to_csv(filename, index=False)
                print(f"✓ Saved {filename}")
    ...
    def plot_results(self):
        ...
        if hasattr(self, 'test_predictions') and self.test_data is not None:
            for col in self.target_columns:
                if col in self.test_targets.columns:
                    ...
                    plt.title(f'{col.title()} Predictions - 2025 Test Data', fontsize=14)
                    plt.xlabel('Date', fontsize=12)
                    plt.ylabel(f'{col.title()}', fontsize=12)
                    plt.legend()
                    plt.grid(True, alpha=0.3)
                    plt.tight_layout()
                    plt.savefig(f'{plots_dir}/{col}_2025_predictions.png', dpi=300, bbox_inches='tight')
                    plt.close()
                    print(f"✓ Saved {col}_2025_predictions.png")
        ...
        for col in self.target_columns:
            ...
            plt.title(f'{col.title()} - {title_period} of {self.prediction_years}-Year Predictions', fontsize=14)
            plt.xlabel('Date', fontsize=12)
            plt.ylabel(f'{col.title()}', fontsize=12)
            plt.legend()
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(f'{plots_dir}/{col}_{self.prediction_years}_year_sample.png', dpi=300, bbox_inches='tight')
            plt.close()
            print(f"✓ Saved {col}_{self.prediction_years}_year_sample.png")
            
    def print_summary(self):
        """Print final summary"""
        print("\n" + "="*60)
        print("DEEP LEARNING AUTO REGRESSIVE (DLAR) - SUMMARY")
        print("="*60)
...
def main(prediction_years=50):
    ...
    print("DEEP LEARNING AUTO REGRESSIVE (DLAR) MODELS")
    print(f"Weather Prediction for {prediction_years} Years")
    print("=" * 60)
    
    runner = DLARRunner(prediction_years=prediction_years)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("Usage: python run.py [prediction_years]")
            print("Example: python run.py 5")
            sys.exit(1)
