...
class CalendarWidget(QtWidgets.QWidget):
    def __init__(self, df, targetConditionsColumn="conditions", replaceConditions=True, parent=None):
        super().__init__(parent)
        self.df = df
        self.df["datetime"] = pd.to_datetime(self.df["datetime"])
        self.df["year"] = self.df["datetime"].dt.year
        self.conditionsColumn = targetConditionsColumn
        ...
        if replaceConditions:
            ...
            self.df[targetConditionsColumn] = self.df[targetConditionsColumn].replace(
                {"Overcast" : "Cloudy", 
                "Rain, Overcast" : "Windy", 
                "Partially cloudy" : "Sunny", 
                "Rain, Partially cloudy" : "Rainy"})
    ...
    def apply_weather_color(self, cell_button, date):
        ...
        if date in self.df["datetime"].tolist():
            ...
            # else:
            #     cell_button.setStyleSheet("border: 1px solid gray; padding: 5px;")
            weather_condition = self.df.loc[self.df["datetime"] == date, self.conditionsColumn].iloc[0]
            if weather_condition in weather_colors:
                cell_button.setStyleSheet(f"border: 1px solid gray; padding: 5px; background-color: {weather_colors[weather_condition]};")
            else:
                cell_button.setStyleSheet("border: 1px solid gray; padding: 5px;")
    ...
    def cell_clicked(self):
        ...
        if date:
            ...
            if date in self.df["datetime"].tolist():
                if "sensor_windspeed" in self.df.columns:
                    condition = self.df.loc[self.df["datetime"] == date, [self.conditionsColumn, "tempmax", "tempmin", "temp", "humidity", "windspeed", "sealevelpressure", "sensor_windspeed" ,"sensor_pressure" ,"sensor_temperature" , "sensor_humidity"]].iloc[0] 
                    sensor_windspeed = condition["sensor_windspeed"]
                    sensor_pressure = condition["sensor_pressure"]
                    sensor_temperature = condition["sensor_temperature"]
                    sensor_humidity = condition["sensor_humidity"]
                else:
                    condition = self.df.loc[self.df["datetime"] == date, [self.conditionsColumn, "tempmax", "tempmin", "temp", "humidity", "windspeed", "sealevelpressure"]].iloc[0]
                # weather = condition["conditions"]
                weather = condition[self.conditionsColumn]
                ...
                if self.conditionsColumn == "predictionsClass":
                    if "sensor_windspeed" in self.df.columns:
                                        info_text += (
                                            f"Date: {date.strftime('%Y-%m-%d')}\n"
                                            f"Weather: {weather}\n"
                                            "Sensor: \n"
                                            f"Avg Temp: {sensor_temperature}\n"
                                            # f"Max Temp: {sensor_temperature}\n"
                                            # f"Min Temp: {sensor_temperature}\n"
                                            f"Wind Speed: {sensor_windspeed}\n"
                                            f"Pressure: {sensor_pressure}\n"
                                            
                                            f"Humidity: {sensor_humidity}\n"
                                        )
                else:
                    info_text += (
                        f"Date: {date.strftime('%Y-%m-%d')}\n"
...
if __name__ == "__main__":
    # df = pd.read_csv("Data/Model Output.csv", "conditions")
    df = pd.read_csv("Data/Jan 2025 to Feb 2025_PREDICTIONS.csv")
    # print(df)
    app = QtWidgets.QApplication(sys.argv)
    window = CalendarWidget(df, "predictionsClass", False)
    window.show()
    sys.exit(app.exec_())
